...
<div class="wpcargo-track wpcargo">
	<form method="post" name="wpcargo-track-form" action="<?php echo esc_html( $get_action ); ?>">
		...
		<div class="containertrack">
			...
			<div class="inputtrack">				
				<input class="input_track_num" type="text" name="<?php echo wpcargo_track_meta(); ?>" value="<?php echo esc_html($shipment_number); ?>" autocomplete="off" placeholder="<?php echo apply_filters('wpcargo_tn_placeholder', esc_html__('Enter Tracking Number', 'wpcargo' ) ); ?>" required>
				<span>#</span>
				<?php do_action('wpcargo_add_form_fields'); ?>
			</div>
			<div class="submit-track">
				<input id="submit_wpcargo" class="wpcargo-btn wpcargo-btn-primary" name="wpcargo-submit" type="submit" value="<?php echo apply_filters('wpcargo_tn_submit_val', esc_html__( 'TRACK RESULT', 'wpcargo' ) ); ?>">
			</div>
		</div>				
