 ...
 * @since 1.0.0
 */
class FAEL_Media_Uploader extends FAEL_Widget_Base {
    ...
    public function get_title() {
        return __( 'UF Media Uploader', 'fael' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => __( 'Media Uploader', 'fael' ),
            ]
        );
        $this->add_control(
            'description',
    ...
    protected function render() {
        ...
        <div id="<?php echo $s['element_id']; ?>" class="fael_media_uploader position-relative form-group <?php echo $s['element_class']; ?>">
            ...
            <img id="frontend-image" :src="fael_forms['<?php echo $s['form_handle']; ?>']['<?php echo $s['name']; ?>'].value" />
            <input id="frontend-button" type="button"
                   class="form-control"
                   value="<?php _e( 'Upload', 'fael' ); ?>"
                   @click="open_uploader('<?php echo $s['form_handle']; ?>', '<?php echo $s['name']; ?>')"
            >
            <template v-if="fael_forms['<?php echo $s['form_handle']; ?>']['<?php echo $s['name']; ?>'].value">
                <input type="button"
                       class="form-control"
                       value="<?php _e( 'Remove', 'fael' ); ?>"
                       @click="remove_media('<?php echo $s['form_handle']; ?>', '<?php echo $s['name']; ?>')"
                >
            </template>
            <input type="text"
...
\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new FAEL_Media_Uploader() );
